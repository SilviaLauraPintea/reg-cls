...
def create_FDS_train_subset_id(args):
    ...
    select_idx = np.random.choice(
        a=list(range(train_depth_values.size(0))), size=600, replace=False
    )
...
def create_FDS_train_subset(args):
    ...
    frame.iloc[select_id].to_csv(
        os.path.join(args.data_dir, "nyu2_train_FDS_subset.csv"),
        index=False,
        header=False,
    )
...
def create_balanced_testset(args):
    ...
    test_bins_number, _ = np.histogram(
        a=test_depth_values_flatten, bins=100, range=(0.0, 10.0)
    )

    select_pixel_num = min(test_bins_number[test_bins_number != 0])
    test_depth_values_flatten_bins = np.array(
        list(map(lambda v: get_bin_idx(v), test_depth_values_flatten))
    )

    test_depth_flatten_mask = np.zeros(
        test_depth_values_flatten.shape[0], dtype=np.uint8
    )
    for i in range(7, 100):
        bucket_idx = np.where(test_depth_values_flatten_bins == i)[0]
        select_bucket_idx = np.random.choice(
            a=bucket_idx, size=select_pixel_num, replace=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--losstype", type=str, default="mse", choices=["mse"])
