...
class FDS(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "running_mean", torch.zeros(bucket_num - bucket_start, feature_dim)
        )
        self.register_buffer(
            "running_var", torch.ones(bucket_num - bucket_start, feature_dim)
        )
        self.register_buffer(
            "running_mean_last_epoch",
            torch.zeros(bucket_num - bucket_start, feature_dim),
        )
        self.register_buffer(
            "running_var_last_epoch", torch.ones(bucket_num - bucket_start, feature_dim)
        )
        self.register_buffer(
        ...
        self.register_buffer(
            ...
            torch.ones(bucket_num - bucket_start, feature_dim),
        )
        self.register_buffer(
            "num_samples_tracked", torch.zeros(bucket_num - bucket_start)
        )
    ...
    def _get_bucket_idx(self, label):
        ...
        return max(
            min(int(label * np.float32(10)), self.bucket_num - 1), self.bucket_start
        )
    ...
    def _update_last_epoch_stats(self):
        ...
        assert (
            self.smoothed_mean_last_epoch.shape == self.running_mean_last_epoch.shape
    ...
    def update_last_epoch_stats(self, epoch):
        if epoch == self.epoch + 1:
            ...
            logging.info(
                f"Updated smoothed statistics of last epoch on Epoch [{epoch}]!"
            )
        ...
        for bucket in np.unique(buckets):
            ...
            factor = (
                ...
                if self.momentum is not None
                    ...
                    1
                    - curr_num_sample
                    / float(self.num_samples_tracked[bucket - self.bucket_start])
                )
            )
            factor = 0 if epoch == self.start_update else factor
            self.running_mean[bucket - self.bucket_start] = (
                1 - factor
            ) * curr_mean + factor * self.running_mean[bucket - self.bucket_start]
            self.running_var[bucket - self.bucket_start] = (
                1 - factor
            ) * curr_var + factor * self.running_var[bucket - self.bucket_start]
            ...
            torch.stack(
                [
                    torch.min(
                        torch.stack(
                            [
                                torch.floor(labels * torch.tensor([10.0]).cuda()).int(),
                                torch.zeros(labels.size(0))
                                .fill_(self.bucket_num - 1)
                                .int()
                                .cuda(),
                            ],
                            0,
                    ...
                    torch.zeros(labels.size(0)).fill_(self.bucket_start).int().cuda(),
                ],
                0,
            ),
            0,
