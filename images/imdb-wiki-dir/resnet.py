...
class ResNet(nn.Module):
    def __init__(
        ...
        # If the number of classes is not 0, add a classification head.
        if self.cls_num > 0:
            self.linear_cls = nn.Linear(512 * block.expansion, self.cls_num)
    ...
    def forward(self, x, targets=None, epoch=None):
        ...
        x_reg = self.linear(encoding_s)

        # Output the classes as well
        if self.cls_num > 0:
            x_cls = self.linear_cls(encoding_s)
        else:
            x_cls = None

        if self.training and self.fds:
            return x_reg, x_cls, encoding
        else:
            return x_reg, x_cls
