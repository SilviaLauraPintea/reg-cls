        ...
        for label in torch.unique(labels):
            ...
            factor = (
                ...
                if self.momentum is not None
                    ...
                    1
                    - curr_num_sample
                    / float(self.num_samples_tracked[int(label - self.bucket_start)])
                )
            )
            factor = 0 if epoch == self.start_update else factor
            self.running_mean[int(label - self.bucket_start)] = (
                1 - factor
            ) * curr_mean + factor * self.running_mean[int(label - self.bucket_start)]
            self.running_var[int(label - self.bucket_start)] = (
                1 - factor
            ) * curr_var + factor * self.running_var[int(label - self.bucket_start)]
