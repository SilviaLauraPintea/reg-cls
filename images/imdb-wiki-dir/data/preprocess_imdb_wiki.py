...
def make_balanced_testset(
    db="imdb_wiki", max_size=150, seed=666, verbose=True, vis=False, save=True
):
    file_path = join(BASE_PATH, "meta", f"{db}.csv")
...
def make_balanced_allset(
    db="imdb_wiki", max_size=150, seed=666, verbose=True, vis=True, save=True
):
    file_path = join(BASE_PATH, "meta", f"{db}.csv")
    df = pd.read_csv(file_path)
    df["age"] = df.age.astype(int)
    ...
    for value in range(121):
        ...
        val_set += list(curr_data[:curr_size])
        test_set += list(curr_data[curr_size : curr_size * 2])
        train_set += list(curr_data[curr_size*2 : curr_size*5])

        print(len(val_set), len(test_set), len(train_set))
    ...
    combined_set.update(dict(zip(test_set, ["test" for _ in range(len(test_set))])))
    print("???", combined_set)
    combined_set.update(dict(zip(train_set, ["train" for _ in range(len(train_set))])))
